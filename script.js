...
// Global Değişkenler
let myMap, aracSheetName, tumGorevler = [], tumPlacemarks = [], clusterer;
...
function initMapAndData() {
    ...
    // Kümeleyiciyi (clusterer) en başta bir kere oluştur
...
function renderHarita(gorevListesi) {
    tumPlacemarks = []; // Pin referans listesini sıfırla
    clusterer.removeAll(); // Kümeleyicinin içini boşalt
    ...
    if (tumPlacemarks.length > 0) {
        // Pinleri kümeleyiciye ekle
        clusterer.add(tumPlacemarks);
        // Haritanın sınırlarını, kümeleyicinin içindeki pinlere göre ayarla
...
function filtrele() {
    // Bu fonksiyon artık pinleri silip eklemiyor, sadece stillerini değiştiriyor.
    const secilenMahalle = document.getElementById('mahalle-filtre').value;
    const boundsToShow = [];
    
    // Pinlerin görünürlüğünü değil, stilini değiştiriyoruz
    ...
    // Listedeki kartları gizle/göster
    document.querySelectorAll('#gorev-listesi-tam .gorev-karti').forEach(kart => {
        kart.style.display = (secilenMahalle === 'TUMU' || kart.dataset.mahalle === secilenMahalle) ? 'block' : 'none';
    });
