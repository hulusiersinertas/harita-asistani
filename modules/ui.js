...
const UI = {
    ...
    render: function() {
        ...
        this.filtrele(); // İlk yüklemede ve güncellemelerde filtreyi uygula
    },
    ...
    renderDetayPaneli: function(rowIndex) {
        ...
        this.elements.gorevDetay.innerHTML = `<h3>${gorev.adSoyad}</h3><p>${gorev.tamAdres}</p><div class="buton-grup">${navButon}<button class="buton verildi-buton" onclick="UI.updateGorev(${gorev.rowIndex}, 'Verildi', '${adSoyadEscaped}')">Verildi</button><button class="buton evde-yok-buton" onclick="UI.updateGorev(${gorev.rowIndex}, 'Evde Yok', '${adSoyadEscaped}')">Evde Yok</button><button class="buton diger-buton" onclick="UI.updateGorev(${gorev.rowIndex}, 'Adres Yanlış', '${adSoyadEscaped}')">Adres Yanlış</button></div>`;
    },

    // Filtreleme işlemini yönetir
    filtrele: function() {
        const secilenMahalle = this.elements.mahalleFiltre.value;
        MapManager.filtreleHarita(secilenMahalle); // Harita modülüne filtreleme yapmasını söyle
    ...
    updateGorev: async function(rowIndex, sonuc, adSoyad) {
        ...
        if (gorevIndex > -1) {
            ...
            this.render(); // Arayüzü yeniden çiz
        }
        
        try {
            await API.updateGorevStatus(rowIndex, sonuc); // API modülüne güncelleme yapmasını söyle
        } catch (error) {
...
// updateGorev fonksiyonunu global scope'a taşıyoruz ki HTML içindeki onclick'ler çalışsın
// Bu, modüler yapının küçük bir "hile"sidir.
window.UI = UI;