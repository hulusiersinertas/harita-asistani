...
const UI = {
    ...
    renderTamListe: function(gorevListesi) { this.elements.gorevListesiTam.innerHTML = ''; gorevListesi.forEach(gorev => { const kart = document.createElement('div'); kart.className = 'gorev-karti'; kart.id = `liste-gorev-${gorev.rowIndex}`; kart.dataset.mahalle = gorev.mahalle; kart.onclick = () => this.listedenGorevSec(gorev.rowIndex); if (!gorev.enlem || !gorev.boylam) kart.classList.add('gorev-karti-hatali'); kart.innerHTML = `<h3>${gorev.adSoyad}</h3><p>${gorev.tamAdres}</p>`; this.elements.gorevListesiTam.appendChild(kart); }); },
    renderDetayPaneli: function(rowIndex) { const gorev = AppState.tumGorevler.find(g => g.rowIndex === rowIndex); if (!gorev) { this.elements.gorevDetay.innerHTML = '<p style="color: #888;">Detayları görmek için bir nokta seçin.</p>'; return; } let navButon = `<button class="buton nav-buton" disabled>Konum Yok</button>`; if (gorev.enlem && gorev.boylam) { navButon = `<a href="https://yandex.com.tr/harita/?rtext=~${gorev.enlem},${gorev.boylam}" target="_blank" class="buton nav-buton">Navigasyon</a><button class="buton" style="background-color: #7b1fa2;" onclick="UI.handleDrawRoute(${gorev.rowIndex})">Rota Çiz</button>`; } const adSoyadEscaped = gorev.adSoyad.replace(/'/g, "\\'"); this.elements.gorevDetay.innerHTML = `<h3>${gorev.adSoyad}</h3><p>${gorev.tamAdres}</p><div class="buton-grup">${navButon}</div><div class="buton-grup"><button class="buton verildi-buton" onclick="UI.updateGorev(${gorev.rowIndex}, 'Verildi', '${adSoyadEscaped}')">Verildi</button><button class="buton evde-yok-buton" onclick="UI.updateGorev(${gorev.rowIndex}, 'Evde Yok', '${adSoyadEscaped}')">Evde Yok</button><button class="buton" style="background-color: #fbc02d; color: black;" onclick="UI.handlePhoneCall('${gorev.telefon}')">Ara</button></div>`; },
    
    filtrele: function() {
        const secilenMahalle = this.elements.mahalleFiltre.value;
        MapManager.filtrele(secilenMahalle); // DOĞRU FONKSİYONU ÇAĞIR
