...
const API = {
    ...
    }, // <-- EKSİK OLAN VİRGÜL BURADAYDI

    // E-Tablodan görev verisini çeker ve işleyip döndürür.
    async function fetchSheetData(sheetName) {
    const range = `'${sheetName}'!A4:P`;
    try {
        const response = await gapi.client.sheets.spreadsheets.values.get({ spreadsheetId: AppConfig.SPREADSHEET_ID, range: range });
        return (response.result.values || []).map((row, index) => {
            const hamEnlem = row[AppConfig.SUTUNLAR.ENLEM], hamBoylam = row[AppConfig.SUTUNLAR.BOYLAM];
            ...
            return {
                ...
                adresNotu: row[5] || '', // F Sütunu (index 5)
                miktar: row[6] || '', // G Sütunu (index 6)
    ...
    } catch (error) { console.error("API.fetchSheetData hatası:", error); throw error; }
},
